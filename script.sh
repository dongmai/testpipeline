...
# sed "s#BUILD_NUMBER#$IMAGE_TAG#g" task-definition.json
# sed "s#REPOSITORY_URI#$REPOSITORY_URI#g" task-definition.json
# sed "s#ROLE_ARN#$ROLE_ARN#g" task-definition.json
# sed "s#FAMILY#$FAMILY#g" task-definition.json
# sed "s#NAME#$NAME#g" task-definition.json
...
aws ecs update-service --cluster "${CLUSTER_NAME}" --service "${SERVICE_NAME}" --task-definition "${TASK_DEFINITION_NAME}":"${REVISION}" --desired-count "${DESIRED_COUNT}" --debug
